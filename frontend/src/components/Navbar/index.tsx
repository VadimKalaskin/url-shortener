...
export default function Navbar({screen, setScreen}: Props) {
	...
	return (
		<section className={'w-full py-4 border-b px-10 flex items-center justify-center gap-4'}>
			<button onClick={() => handleClick('index')} className={screen === 'index' ? 'font-bold' : ""}>Главная</button>
			<button onClick={() => handleClick('list')} className={screen === 'list' ? 'font-bold' : ""}>Все ссылки</button>
		</section>
	)
