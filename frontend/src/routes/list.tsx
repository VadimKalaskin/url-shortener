...
export default function UrlList() {
	...
	if (urlList.length === 0)
		return <p className="text-center text-gray-500 mt-10">Нет доступных ссылок</p>;
	...
	const InfoBlock = () => {
		...
		if (isLoading) return <h1 className="text-2xl font-bold mb-4 text-center">Загрузка...</h1>;

		return (
			<div className={'flex flex-col gap-2'}>
				<h2>Детальная информация о ссылке {urlInfo?.alias}</h2>
				<span>
					Количество кликов: <b>{urlData?.clickCount}</b>
				</span>
				{urlData?.clickCount !== 0 && (
					<>
						<span>Последние 5 IP адресов: </span>
						<ul>{urlData?.clicks.map((item, i) => <li key={i}>{item.ipAddress}</li>)}</ul>
					</>
				)}
			</div>
		);
